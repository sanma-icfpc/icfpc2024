...
if __name__ == '__main__':
    if len(sys.argv) == 2:
        ...
        with open(sys.argv[1], 'r') as fi:
            ...
            num, path = mo.groups()
            num = int(num)
            compressed = compress_lambdaman_base4(num, path)
            if len(org) > len(compressed):
                print(f'lambdaman{num} SUCCEEDED {len(org)}->{len(compressed)}')
                with open(os.path.splitext(sys.argv[1])[0] + '.base4.txt', 'w') as fo:
                    fo.write(compressed)
            else:
                print(f'lambdaman{num} NO IMPROVEMENT {len(org)}->{len(compressed)}')
    else:
        #unittest.main()
        ...
        with open('lambdaman1_compress.txt', 'w') as fo:
